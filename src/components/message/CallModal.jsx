...
const CallModal = () => {
    ...
    // End Call
    const addCallMessges = useCallback((call, times, disconnect) => {
    ...
    useEffect(() => {
        if(answer){
            ...
            const timer = setTimeout(() => {
                ...
                addCallMessges(call, 0)
                dispatch({type: GLOBALTYPES.CALL, payload: null})
            }, 15000)
            return () => clearTimeout(timer);
        }
    }, [dispatch, answer, call, socket, addCallMessges])

    useEffect(() => {
        socket.on('endCallToClient', data => {
            tracks && tracks.forEach(track => track.stop())
            addCallMessges(data, data.times)
            dispatch({ type: GLOBALTYPES.CALL, payload: null })
        })

        return () => socket.off('endCallToClient');
    }, [socket, dispatch, tracks, addCallMessges])
    ...
    useEffect(() => {
        socket.on('callerDisconnect', () => {
            ...
            let times = answer ? total : 0;
            addCallMessges(call, times, true)
    ...
    }, [socket, tracks, dispatch, call, addCallMessges, answer, total])
    ...
    return (
        <div className="call_modal">
            ...
            <div className="show_video" style={{
                ...
                <video ref={yourVideo} className="your_video" />
